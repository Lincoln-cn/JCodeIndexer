...
public class Indexer {
    ...
    /**
     * TypeScript 文件 diff 索引：类似 Kotlin，使用 TypeScriptParserAdapter
     */
    private void indexTypeScriptFile(String relativePath, Path filePath) throws Exception {
        ParseResult parsed = typeScriptParser.parse(relativePath, filePath);
        ...
        log.debug("TypeScript diff 完成: {} (符号: {}增{}删, 块: {}增{}删)",
            relativePath, symbolsToInsert.size(), symbolIdsToDelete.size(),
            chunksToInsert.size(), chunkIdsToDelete.size());
    }
